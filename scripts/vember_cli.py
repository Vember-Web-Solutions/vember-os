...
class VemberCLI:
    ...
    def __init__(self):
        ...
        self.menu_items = [
			...
			},
			{
				"label": "Run Overseer",
				"cmd": "OVERSEER",
				"desc": "Monitor system diagnostics",
			},
			{
				"label": "Commands & Help",
				"cmd": "CODEX",
			...
			{
				"label": f"Windfall Engine [bold {Theme.WARNING}](New)[/]",
				"cmd": "WINDFALL",
				"desc": "AI-Driven UI Generation Matrix",
			},
			{
    ...
    def _generate_layout(self) -> Align:
        ...
        action_bar = f"[{Theme.ACCENT}]Navigate: [↑/↓][/] | [{Theme.ACTION}]Launch: [Enter][/]"
    ...
    def _execute(self, target_id: str) -> None:
        ...
        if target_id == "ARCHITECT":
            from scripts.architect import Architect
            from rich.live import Live

            self.active_node = "Architectural Audit"
            self.node_output = "Igniting Scanner..."

            # Use our new _generate_layout inside the Live context
            with Live(
        ...
        if target_id == "CODEX":
            ...
            # Simple persistence for Codex
            while True:
                self._render_menu()  # Or update layout
                # Add input capture for Codex return...
                break

        # Fallback for standard shell strings
        os.system("cls" if os.name == "nt" else "clear")
        subprocess.run(target_id, shell=True)
