...
class Windfall:
    ...
    def _dispatch_tui(self, component_type: str, data: Dict[str, Any]) -> str:
        """
        Internal dispatcher for Terminal-based rendering calls.
        
        Args:
            component_type: The specific TUI component to generate.
            data: The system state data to inject into the template.
        """
        if component_type == "header":
            # Pass data directly to our specialized TUI box-drawer
            title = data.get("title", "Vember-OS")
            status = data.get("status", "Unknown")
            return self.tui.render_box(title=title, content=f"Status: {status}")
